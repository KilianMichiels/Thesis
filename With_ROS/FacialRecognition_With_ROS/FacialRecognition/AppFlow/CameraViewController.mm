...
- (IBAction)moveUpPressed:(id)sender {
    [_attentionTimer invalidate];
    NSLog(@"Moving up");
    [[DoubleRoboticsControl sharedInstance] poleUp];
...
- (IBAction)moveDownPressed:(id)sender {
    [_attentionTimer invalidate];
    NSLog(@"Moving down");
    [[DoubleRoboticsControl sharedInstance]poleDown];
...
- (IBAction)moveStop:(id)sender {
    ...
    NSLog(@"Stop moving");
    [[DoubleRoboticsControl sharedInstance] poleStop];
...
-(void)performSegueToApresFR{
    if(SEGUES_ENABLED){
        @try {
            [_ROSController publishViewFlow:@"Going to ApresFR..."];
