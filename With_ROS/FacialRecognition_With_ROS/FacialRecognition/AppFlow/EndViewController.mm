...
-(void)performSegueToStart{
    if(SEGUES_ENABLED){
        @try {
            // Update statistics:
            [_ROSController publishSuccess:_success];
            [_ROSController publishFail:_fail];
            
            // Reset parameters:
            [_ROSController publishFacialRecogntionMessage:@""];
            [_ROSController publishUsername:@""];
            [_ROSController publishUserTag:@""];
            [_ROSController publishFirstTime:NO];
