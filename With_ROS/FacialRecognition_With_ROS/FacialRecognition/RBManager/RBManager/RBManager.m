...
#ifdef DEBUG
        NSLog(@"RB Manager -- Socket already open");
        if ([self.delegate respondsToSelector:@selector(managerAlreadyConnected:)]){
            [self.delegate managerAlreadyConnected:self];
        }
#endif
    } else {
#ifdef DEBUG
        NSLog(@"RB Manager -- connecting -- %@", socketHost);
#endif
...
-(void)disconnect {
    dispatch_queue_t manager_queue = dispatch_queue_create("com.KM.ROSApplication.manager.queue", NULL);
    
    dispatch_async(manager_queue, ^(void) {
...
-(void)sendData:(NSDictionary*)data {
    ...
    if (jsonError){
        NSLog(@"RB Manager -- JSON error -- %@", [jsonError localizedDescription]);
        if(TESTING_VARIABLES){
...
-(void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message {
    ...
    if (error){
        ...
        NSLog(@"RB Manager -- JSON error -- %@", [error localizedDescription]);
    }
    else {
...
-(void)webSocket:(SRWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean {
    ...
    NSLog(@"RB Manager -- Socket closed -- %@ -- Code %ld -- %@", [webSocket.url description], (long)code, reason);
#endif
    self.connected = NO;
    self.socket = nil;
...
-(void)webSocket:(SRWebSocket *)webSocket didFailWithError:(NSError *)error {
    ...
    NSLog(@"RB Manager -- Socket failed -- %@ -- %@", [webSocket.url description], [error localizedDescription]);
#endif
    self.connected = NO;
    self.socket = nil;
    [[NSNotificationCenter defaultCenter] postNotificationName:@"rbmanager/disconnected" object:nil];
...
-(void)webSocketDidOpen:(SRWebSocket *)webSocket {
    ...
    NSLog(@"RB Manager -- Socket open -- %@", [webSocket.url description]);
#endif
