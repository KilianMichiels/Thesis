...
extern NSString * photoTakenByKairos;
extern UIImage * photoTakenByKairos_Image;
...
- (void)showUI
{
    ...
    if(TIMERS_ENABLED){
        ...
        _timer = [NSTimer scheduledTimerWithTimeInterval:TIMER_TIME_FACIAL_RECOGNITION
                                                  target:self
...
//    NSLog(@"Converting image to JPEG image");
...
//    NSLog(@"Converting face to JPEG face");
//    image = [UIImage imageWithCGImage:[image CGImage] scale:[image scale] orientation: UIImageOrientationDown];
    ...
    NSData *imageData = UIImageJPEGRepresentation(image, 1.0);
    if ([imageData respondsToSelector:@selector(base64EncodedStringWithOptions:)]) {
        NSLog(@"Recognize with image: base64");
        base64 = [imageData base64EncodedStringWithOptions:kNilOptions];  // iOS 7+
    } else {
        base64 = [imageData base64Encoding];                              // pre iOS7
    }
...
- (NSData *)compressCIImage:(CIImage *)coreImage {
    ...
    NSLog(@"Converting face to JPEG face");
    face = [UIImage imageWithCGImage:[face CGImage] scale:[face scale] orientation: UIImageOrientationDown];
    
    /////////////////////////////////////////////////////////////////
    //                       TURNING THE IMAGE                     //
    /////////////////////////////////////////////////////////////////
    
    UIImage * screenshot_correct = face;
    
    photoTakenByKairos_Image = face;
    
    NSData *compressedImageData = UIImageJPEGRepresentation(face, 1.0f);
