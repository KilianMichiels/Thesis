...
- (BOOL)subscribeToControls{
    ...
    _controlsSubscriber.throttleRate = 0.0000001;
...
- (BOOL)subscribeToFrameRate{
    NSLog(@"Subscribing MotionDetection");
    
    @try {
        [_frameRateSubscriber unsubscribe];
        NSLog(@"Unsubscribed.");
    } @catch (NSException *exception) {
        NSLog(@"Could not unsubscribe.");
    }
    
    NSString * subscriber = @"/Double_1/frameRate";
    _frameRateSubscriber = [_manager addSubscriber:subscriber responseTarget:self selector:@selector(frameRateMessageUpdate:) messageClass: [FloatMessage class]];
    
    _frameRateSubscriber.throttleRate = 100;
    
    @try {
        [_frameRateSubscriber subscribe];
        NSLog(@"Subscribed!");
        return YES;
    } @catch (NSException *exception) {
        NSLog(@"Could not subscribe.");
...
- (BOOL)subscribeToSpeed{
    NSLog(@"Subscribing MotionDetection");
    
    @try {
        [_speedSubscriber unsubscribe];
        NSLog(@"Unsubscribed.");
    } @catch (NSException *exception) {
        NSLog(@"Could not unsubscribe.");
    }
    
    NSString * subscriber = @"/Double_1/speed";
    _speedSubscriber = [_manager addSubscriber:subscriber responseTarget:self selector:@selector(speedMessageUpdate:) messageClass: [FloatMessage class]];
    
    _speedSubscriber.throttleRate = 100;
    
    @try {
        [_speedSubscriber subscribe];
        NSLog(@"Subscribed!");
        return YES;
    } @catch (NSException *exception) {
        NSLog(@"Could not subscribe.");
...
/*
 ...
 Message Class: FloatMessage
 */
- (void)frameRateMessageUpdate:(FloatMessage *)message{
    NSLog(@"\n--------------------------------------------\nframeRateMessageUpdate: %@\n--------------------------------------------\n", message.data);
    [self.delegate messageUpdate_FrameRate:message];
}
...
/*
 ...
 Message Class: FloatMessage
 */
- (void)speedMessageUpdate:(FloatMessage *)message{
    NSLog(@"\n--------------------------------------------\nspeedMessageUpdate: %@\n--------------------------------------------\n", message.data);
    [self.delegate messageUpdate_Speed:message];
}
...
- (BOOL)initialise
{
    ...
    if(![self subscribeToFrameRate]){
        return NO;
    }
    if(![self subscribeToSpeed]){
        return NO;
    }
    [self.delegate MainROSControlDidConnect];
    return YES;
...
- (void)sendTestObjects{
    for(int i = 0 ; i < 1000 ; i++){
        //dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(i * 0.005 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [self publishFirmwareVersion:[NSString stringWithFormat:@"%f", timeInSeconds]];
            //////////////////////////////////
