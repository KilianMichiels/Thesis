...
@property int numberOfMessagesReceivedCounter;
@end
...
- (void)viewDidLoad {
    ...
    _numberOfMessagesReceivedCounter = 0;
...
- (IBAction)TestButtonPressed:(id)sender {
    [self updateLog:@"\nSending messages (N = 1000) --> Check firmware version for Delay."];
...
-(void)messageUpdate_kickStandState:(IntMessage *)message{
    _numberOfMessagesReceivedCounter += 1;
    [self updateNumberOfMessages];
...
-(void)messageUpdate_FrameRate:(FloatMessage *)message{
    _numberOfMessagesReceivedCounter += 1;
    [self updateNumberOfMessages];
    
    NSLog(@"ROS -- Frame Rate updated");
    [self updateLog:[NSString stringWithFormat:@"\nROS -- Frame Rate updated -- %f", [message.data floatValue]]];
    [self.updateTimer invalidate];
    self.updateTimer = nil;
    _updateTimer = [NSTimer scheduledTimerWithTimeInterval:[message.data floatValue] target:self selector:@selector(sendImage) userInfo:nil repeats:YES];
}

- (void)messageUpdate_Speed:(FloatMessage *)message{
    _numberOfMessagesReceivedCounter += 1;
    [self updateNumberOfMessages];
    
    NSLog(@"ROS -- Speed updated");
    [self updateLog:[NSString stringWithFormat:@"\nROS -- Speed updated -- %f", [message.data floatValue]]];
    
    float messageData = [message.data floatValue];
    if (messageData >= 0 && messageData <= 100){
        [_speedSlider setValue:(messageData/100)*5];
        _doubleSpeed = 0.2*[_speedSlider value];
    }
}
...
- (void)sendImage{
    ...
    dispatch_async(backgroundQueue, ^(void) {
        ...
        NSData * sendImage = UIImageJPEGRepresentation(screenshot, 0.3);
...
-(void) updateNumberOfMessages{
    NSLog(@"Updating number of messages received.");
    _numberOfMessagesReceived.text = @"";
    _numberOfMessagesReceived.text = [NSString stringWithFormat:@"%d", _numberOfMessagesReceivedCounter];
    NSLog(@"Done");
}
