...
- (void)viewDidLoad
{
    ...
    [upButton addTarget:self
               action:@selector(moveDoubleUp)
     forControlEvents:UIControlEventTouchDown];
    
    [upButton addTarget:self
                 action:@selector(moveDoubleStop)
       forControlEvents:UIControlEventTouchUpInside];
    
    [upButton setTitle:@"Move camera up" forState:UIControlStateNormal];
    upButton.frame = CGRectMake(0, 771.0, 384.0, 100.0);
    upButton.backgroundColor = [UIColor colorWithHue:0.568603 saturation: 1.041124 brightness:0.574885 alpha:0.200000];
    [upButton setFont:[UIFont fontWithName:@"HelveticaNeue-UltraLight" size:40.0]];
    [[self view] addSubview:upButton];
    
    UIButton *downButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [downButton addTarget:self
                 action:@selector(moveDoubleDown)
       forControlEvents:UIControlEventTouchDown];
    ...
    [downButton setTitle:@"Move camera down" forState:UIControlStateNormal];
    downButton.frame = CGRectMake(0, 879.0, 384.0, 100.0);
    [downButton setFont:[UIFont fontWithName:@"HelveticaNeue-UltraLight" size:40.0]];
    downButton.backgroundColor = [UIColor colorWithHue:0.568603 saturation: 1.041124 brightness:0.574885 alpha:0.200000];
    [[self view] addSubview:downButton];
    
}

- (void)moveDoubleUp{
    NSLog(@"FR -- Move camera up pressed");
    [[DRDouble sharedDouble] poleUp];
}

- (void)moveDoubleDown{
    NSLog(@"FR -- Move camera down pressed");
    [[DRDouble sharedDouble] poleDown];
}

- (void)moveDoubleStop{
    NSLog(@"FR -- Move camera stop pressed");
...
- (void)showStillWithImage:(CIImage*)ciimg
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^(void) {
        ...
        _stillImageView.transform = CGAffineTransformMakeRotation(M_PI);
...
-(AVCaptureDevice *)frontFacingCameraIfAvailable {
    ...
    NSLog(@"frontFacingCameraIfAvailable");
       ...
       fromConnection:(AVCaptureConnection *)connection
{
    ...
    @autoreleasepool {
        ...
        //        NSLog(@"\n-------------------------\nCurrent Orientation: %ld\n-------------------------", (long)curDeviceOrientation);
